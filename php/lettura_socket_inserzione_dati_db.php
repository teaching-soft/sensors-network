...
do {
        ...
        if (socket_getpeername($clientSocket, $remoteIp, $remotePort)) {
                echo "\nConnessione da $remoteIp:$remotePort-->";
        } else {
                echo "\nImpossibile ottenere l'indirizzo IP del client: " . socket_strerror(socket_last_error($clientSocket)) . "\n";
        }
...
function save_data_to_db($dati_arrivati)
{
        ...
        if((strpos($dati_arrivati,"[") === false) || (strpos($dati_arrivati,"]") === false)){
                echo "\nFormato dati arrivati non corretto, manca [ o ]:" . $dati_arrivati . "\n";
                return;
        }
        // Ripulisce i dati
        $buffer_tmp = str_replace ("[" , "" ,$dati_arrivati);
        $values = str_replace ("]" , "" ,$buffer_tmp);
        $fields = explode(":",trim($values));
        $locazione = $fields[0];
        $temperature = $fields[1];
        date_default_timezone_set('Europe/Rome');
        $data = date("Y/m/d");
        $time = date("H:i:s");
        echo "Dati:" . $data . " " . $time . " " . $locazione . " " . $temperature . " | ";
...
try {
                ...
                $db_handle = new PDO($connect_str,DB_USER,DB_PW);
                   ...
                   $query = "INSERT INTO temperature_tbl (date, time,locazione,temperature) VALUES ('" . $data . "','" . $time . "','" . $locazione  . "','" . $temperature  . "')";
              ...
              echo "Dati inseriti:" . $data . " " . $time . " " . $locazione . " " . $temperature ."\n";
}
?>
